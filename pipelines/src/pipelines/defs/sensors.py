...
def sensors(context: dg.SensorEvaluationContext) -> dg.SensorResult:
    ...
    try:
        ...
        context.log.info(f'Found {len(seen_hashes)} previously processed file hash(es) from materialization history')
    ...
    new_files_with_hashes = [
        {'path': current_file_hashes[h], 'hash': h}
    ...
    for item in new_files_with_hashes:
        context.log.info(f'  - {Path(item["path"]).name} (hash: {item["hash"][:12]}...)')
    ...
    return dg.RunRequest(
        run_config={
            'ops': {
                'assetsRaw': {
                    'config': {
                        'new_files': new_files_with_hashes
                    }
                }
            }
        },
